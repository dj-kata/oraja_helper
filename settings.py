...
class SettingsWindow:
    def __init__(self, parent, config, on_close_callback=None):
        ...
        self.enable_folder_updates_var = tk.BooleanVar(
            value=self.config.enable_folder_updates
        )
        self.autoload_offset_var = tk.IntVar(value=self.config.autoload_offset)
        self.enable_register_conditions_var = tk.BooleanVar(
            value=self.config.enable_register_conditions
        )
        self.nglist_vars = {}
    ...
    def setup_ui(self):
        ...
        ttk.Label(
            autoload_offset_frame,
            text="何時間前までのリザルトを自動で読み込むか:",
            width=45,
        ).pack(side=tk.LEFT)
        self.autoload_offset_entry = ttk.Entry(
            autoload_offset_frame, textvariable=self.autoload_offset_var, width=5
        )
        self.autoload_offset_entry.pack(side=tk.LEFT, padx=(5, 0))

        self.button_load_oraja_log = ttk.Button(
            autoload_offset_frame, text="過去ログ取得", command=self.load_oraja_log
        ).pack(side=tk.RIGHT)

        # ツイート設定セクション
        tweet_frame = ttk.LabelFrame(
            self.scrollable_frame, text="ツイート設定", padding="10"
        )
        ...
        folder_frame = ttk.LabelFrame(
            self.scrollable_frame, text="監視設定", padding="10"
        )
        ...
        self.oraja_path_entry = ttk.Entry(
            oraja_path_frame, textvariable=self.oraja_path_var, state="readonly"
        )
        self.oraja_path_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))

        ttk.Button(oraja_path_frame, text="変更", command=self.change_oraja_path).pack(
            side=tk.RIGHT
        )
        ...
        self.player_path_entry = ttk.Entry(
            player_path_frame, textvariable=self.player_path_var, state="readonly"
        )
        self.player_path_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))

        ttk.Button(
            player_path_frame, text="変更", command=self.change_player_path
        ).pack(side=tk.RIGHT)

        # WebSocket設定セクション
        websocket_frame = ttk.LabelFrame(
            self.scrollable_frame, text="WebSocket連携設定", padding="10"
        )
        ...
        self.websocket_host_entry = ttk.Entry(
            host_frame, textvariable=self.websocket_host_var
        )
        self.websocket_host_entry.pack(
        ...
        self.websocket_port_entry = ttk.Entry(
            port_frame, textvariable=self.websocket_port_var, width=10
        )
        ...
        self.websocket_password_entry = ttk.Entry(
            password_frame, textvariable=self.websocket_password_var, show="*"
        )
        self.websocket_password_entry.pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=(5, 0)
        )
    ...
    def setup_scrollable_frame(self):
        ...
        self.scrollbar = ttk.Scrollbar(
            self.main_container, orient="vertical", command=self.scroll_canvas.yview
        )
    ...
    def setup_ui_nglist(self):
        ...
        # 難易度表セクションフレーム（高さを制限）
        self.nglist_section = ttk.LabelFrame(
            self.scrollable_frame, text="難易度表選択", padding="10"
        )
        ...
        nglist_scrollbar = ttk.Scrollbar(
            self.nglist_section, orient="vertical", command=self.canvas.yview
        )
        self.nglist_frame = ttk.Frame(self.canvas)

        ttk.Label(
            self.nglist_frame, text="ログに難易度を表示する難易度表", width=30
    ...
    def validate_settings(self):
        ...
        try:
            ...
            if port < 1 or port > 65535:
                messagebox.showerror(
                    "入力エラー", "ポート番号は1-65535の範囲で入力してください。"
                )
                return False
        except tk.TclError:
        ...
        if self.enable_websocket_var.get():
            ...
            if not host:
                messagebox.showerror(
                    "入力エラー",
                    "WebSocket連携を使用する場合、ホスト名を入力してください。",
                )
                return False
    ...
    def on_save(self):
        ...
        try:
            ...
            self.config.enable_register_conditions = (
                self.enable_register_conditions_var.get()
            )
