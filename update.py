...
class Updater:
    ...
    def update_from_url(self, url):
        ...
        p = Path('tmp/oraja_helper')
        failed_list = []
        for f in p.iterdir():
            if f.is_dir():
                subdir=f.relative_to('tmp/oraja_helper')
                os.makedirs(subdir, exist_ok=True)
        for f in p.glob('**/*.*'):
            try:
                base = str(f.relative_to('tmp/oraja_helper'))
                logger.debug(f)
        ...
        shutil.rmtree('tmp/oraja_helper')
        out = ''
...
if __name__ == '__main__':
    ...
    url = f'https://github.com/dj-kata/oraja_helper/releases/download/{ver}/oraja_helper.zip'
    if type(ver) != str:
